...
- [ ] Bump `version` in `config/package-solution.json` before each store upload (currently **1.0.0.28**).
...
Current package version: **1.0.0.28** (update `config/package-solution.json` before each store upload).
