...
- [ ] Bump `version` in `config/package-solution.json` before each store upload (currently **1.0.0.31**).
...
Current package version: **1.0.0.31** (update `config/package-solution.json` before each store upload).
