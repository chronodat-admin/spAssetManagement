...
- [ ] Bump `version` in `config/package-solution.json` before each store upload (currently **1.0.0.25**).
...
Current package version: **1.0.0.25** (update `config/package-solution.json` before each store upload).
