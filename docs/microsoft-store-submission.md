...
- [ ] Bump `version` in `config/package-solution.json` before each store upload (currently **1.0.0.18**).
...
Current package version: **1.0.0.18** (update `config/package-solution.json` before each store upload).
