...
export class SubscriptionService {
  ...
  private async authorizedFetch(
    ...
    try {
      ...
      // AAD token acquisition can fail when the subscription API is not registered
      // as an enterprise application in the tenant. The API accepts unauthenticated
      // browser calls via CORS when server-side AAD validation is not required.
      return fetch(url, init);
    }
  }
  ...
  async sendNotification(params: {
    ...
    if (!response.ok) {
      const body = (await response.json().catch(() => ({}))) as { error?: string };
      throw new Error(body.error ?? `Notification send failed (${response.status})`);
    }
  }
}
