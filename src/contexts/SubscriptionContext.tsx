...
export interface ISubscriptionContextValue {
  ...
  hasAccess: boolean;
  /** True when the API was unreachable and no usable cached status is available. */
  connectivityError: boolean;
  /** True when the displayed status comes from the offline cache (grace window). */
...
export const SubscriptionProvider: React.FC<ISubscriptionProviderProps> = ({
  ...
  const refresh = React.useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // The API was unreachable. Rather than locking out paying customers (or
      // showing a misleading "trial ended" paywall), reuse the last successful
      // status while it is still within the offline grace window.
