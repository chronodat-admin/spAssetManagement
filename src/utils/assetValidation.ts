...
/** @deprecated */
const RISK_STATUS_CHOICES = ASSET_STATUS_CHOICES;
...
export function validateAssetForm(
  ...
  if (!values.title.trim()) {
    return 'Asset/Opportunity title is required.';
  }
  ...
  if (!values.profileId) {
    return 'Asset profile type is required.';
  }
  ...
  const statusOptions = allowedStatuses && allowedStatuses.length > 0 ? allowedStatuses : [...RISK_STATUS_CHOICES];
