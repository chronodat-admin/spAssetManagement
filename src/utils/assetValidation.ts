...
} from '../constants/assetChoices.js';
...
export function validateAssetForm(
  ...
  if (values.categoryId !== undefined && !values.categoryId) {

    return 'Asset category is required.';

  }

  if (values.businessId !== undefined && !values.businessId) {

    return 'Business is required.';

  }

  if (values.profileId !== undefined && !values.profileId) {
  ...
  if (
    ...
    values.likelihood !== undefined &&

    (!values.likelihood ||

      !LIKELIHOOD_CHOICES.includes(values.likelihood as (typeof LIKELIHOOD_CHOICES)[number]))
  ...
  if (
    ...
    values.consequence !== undefined &&

    (!values.consequence ||

      !CONSEQUENCE_CHOICES.includes(values.consequence as (typeof CONSEQUENCE_CHOICES)[number]))
