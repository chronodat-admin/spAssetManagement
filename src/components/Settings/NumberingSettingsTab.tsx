...
const NUMBERING_TABS: Array<{ entityType: NumberingEntityType; label: string }> = [
  ...
  { entityType: 'asset', label: 'Assets' },

  { entityType: 'vendor', label: 'Vendors' },
...
const ENTITY_LABELS: Record<NumberingEntityType, { label: string; description: string }> = {
  ...
  asset: { label: 'Assets', description: 'Auto-numbering for AM_AssetId values on create' },

  vendor: { label: 'Vendors', description: 'Auto-numbering for vendor AM_Code values on create' },

  project: { label: 'Projects', description: 'Auto-numbering for project AM_Code on create' }
...
export const NumberingSettingsTab: React.FC<INumberingSettingsTabProps> = ({
  ...
  const [activeTab, setActiveTab] = React.useState<NumberingEntityType>('asset');
  ...
  return (
    ...
    <div>
      ...
      {resetMessage && (
        ...
        <AppMessageBar intent="success" style={{ marginBottom: tokens.spacingVerticalM }}>
      ...
      <TabList
        ...
        onTabSelect={(_, data) => setActiveTab((data.value as NumberingEntityType) || 'asset')}
