...
import { findAssetByScanValue, parseScannedBarcode } from '../../utils/barcodeUtils';
...
export const BarcodeScannerPage: React.FC<IBarcodeScannerPageProps> = ({ assets, onViewAsset }) => {
  ...
  const handleSearch = (): void => {
    ...
    const parsed = parseScannedBarcode(scanValue);

    setMatched(findAssetByScanValue(assets, parsed));
  ...
  React.useEffect(() => {
    ...
    const startCamera = async (): Promise<void> => {
      ...
      if (!('BarcodeDetector' in window) || !navigator.mediaDevices?.getUserMedia) {
        ...
        setCameraError(t('barcode', 'noCamera', 'Camera scanning is not available in this browser. Enter the code manually.'));
        ...
        const tick = async (): Promise<void> => {
          ...
          try {
            ...
            if (codes[0]?.rawValue) {
              ...
              const parsed = parseScannedBarcode(codes[0].rawValue);

              const asset = findAssetByScanValue(assets, parsed);
